...
$stmt = $koneksi->prepare("SELECT nim, nama, email FROM mahasiswa WHERE email=? LIMIT 1");
$stmt->bind_param("s", $email);
$stmt->execute();
$res = $stmt->get_result();

if (!$res || $res->num_rows !== 1) {
  header("Location: forgot_password_mahasiswa.php?msg=notfound");
  exit;
}
...
$up = $koneksi->prepare("UPDATE mahasiswa SET reset_token_hash=?, reset_token_expire=? WHERE nim=?");
$up->bind_param("ssi", $tokenHash, $expire, $user['nim']);
$up->execute();

// Buat link reset 
$baseUrl = "http://localhost/sistemcdc"; 
$resetLink = $baseUrl . "/mahasiswa_site/resetpassword_mahasiswa.php?token=" . urlencode($token) . "&email=" . urlencode($user['email']);
...
try {
  ...
  $mail->send();
  header("Location: forgot_password_mahasiswa.php?msg=sent");
  exit;
...
}
 {
  ...
  $clr = $koneksi->prepare("UPDATE mahasiswa SET reset_token_hash=NULL, reset_token_expire=NULL WHERE nim=?");
  $clr->bind_param("i", $user['nim']);
  $clr->execute();
